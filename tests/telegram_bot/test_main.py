...
def test_build_dispatcher_registers_all_five_message_routes():
    dispatcher = build_dispatcher(
        store=None, llm=None, sessions=None, stats=None, allowed_user_ids=frozenset()
    )
    names = _handler_names(dispatcher)

    assert "documents_route" in names
    assert "delete_route" in names
    assert "stats_route" in names
...
def test_command_routes_are_registered_before_the_catch_all_text_route():
    ...
    assert names.index("stats_route") < names.index("text_route")
...
def test_build_dispatcher_registers_an_auth_middleware():
    ...
    dispatcher = build_dispatcher(
        store=None, llm=None, sessions=None, stats=None, allowed_user_ids=frozenset({1})
    )
